...
public class CucumberPlant extends BlockCrops
{
	...
	{	
		if(!worldIn.isRemote)
		{
			...
			{
				spawnAsEntity(worldIn, pos, new ItemStack(this.getCrop(), worldIn.rand.nextInt(4) + 1));
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos)
	{
		...
		if(worldIn.getBlockState(pos.down()).getBlock() != Blocks.FARMLAND)
		{
			...
			if(a < 6)
			{
				spawnAsEntity(worldIn, pos, new ItemStack(this.getSeed(), worldIn.rand.nextInt(4) + 1));
			}
			else
			{
				spawnAsEntity(worldIn, pos, new ItemStack(this.getCrop(), worldIn.rand.nextInt(4) + 1));
    ...
    {
		return this.isMaxAge(state) ? new ItemStack(this.getCrop(), rand.nextInt(4) + 1).getItem() : new ItemStack(this.getSeed(), rand.nextInt(4) + 1).getItem();
    }
}