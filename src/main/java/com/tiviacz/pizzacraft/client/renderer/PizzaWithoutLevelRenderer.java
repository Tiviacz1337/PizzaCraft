...
public class PizzaWithoutLevelRenderer extends BlockEntityWithoutLevelRenderer
{
    ...
    public void renderByItem(ItemStack stack, ItemTransforms.TransformType context, PoseStack poseStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay)
    {
        ...
        if(rendershape != RenderShape.INVISIBLE)
        {
            ...
            renderModel(stack, poseStack.last(), buffer.getBuffer(ItemBlockRenderTypes.getRenderType(pState, false)), pState, bakedmodel, combinedLight, combinedOverlay, getModelData(stack));
        }
    }

    public void renderModel(ItemStack stack, PoseStack.Pose pPose, VertexConsumer pConsumer, @Nullable BlockState pState, BakedModel pModel, int pPackedLight, int pPackedOverlay, IModelData modelData) {
        Random randomsource = new Random();
        long i = 42L;

        for(Direction direction : Direction.values()) {
            randomsource.setSeed(42L);
            renderQuadList(stack, pPose, pConsumer, pModel.getQuads(pState, direction, randomsource, modelData), pPackedLight, pPackedOverlay);
        }

        randomsource.setSeed(42L);
        renderQuadList(stack, pPose, pConsumer, pModel.getQuads(pState, (Direction)null, randomsource, modelData), pPackedLight, pPackedOverlay);
    }
